...
def process_silver_financials_table(snapshot_date_str, bronze_fin_directory, silver_fin_directory, spark):
    ...
    # regexp_extract preserves the leading sign so the when(col < 0, None) guard works correctly;
    # the old regexp_replace([^\d.]) stripped "-" before the guard, making it dead code.
    cols_decimal3 = [
        'Annual_Income', 'Monthly_Inhand_Salary', 'Outstanding_Debt',
        'Total_EMI_per_month', 'Amount_invested_monthly', 'Monthly_Balance'
    ]
    for c in cols_decimal3:
        df = df.withColumn(c, F.regexp_extract(F.col(c).cast("string"), r"([-+]?\d*\.?\d+)", 1))
        df = df.withColumn(c, F.round(F.col(c).cast("double"), 3))
        df = df.withColumn(c, F.when(F.col(c) < 0, None).otherwise(F.col(c)))

    # Integer columns — same sign-preservation fix
    cols_integer = [
    ...
    for c in cols_integer:
        df = df.withColumn(c, F.regexp_extract(F.col(c).cast("string"), r"([-+]?\d+)", 1))
